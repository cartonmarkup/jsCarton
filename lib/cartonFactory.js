...
! function () {
  ...
  function prefixify ( name, value ) {
    ...
    if ( has( prefix.selector, name ) ) {
      x =  browser.join( name + ':' + value + '; ' ) + x;
    } 
  ...
  function init ( settings ) {
    ...
    if ( settings.extend ) extend( cartonSettings, settings.extend );
  ...
  Carton.prototype = {
    ...
    parse: function () {
      return {
        ...
        , show: this.show ? this.parseStyles( cartonSettings.show[ this.type ] ) : undefined
        },
        classNames: {
  ...
  function asString ( query ) {
    ...
    return CSSstyle + makeRetinaQuery() + CSSquery;
  }
  ...
  function find ( condition, lookup ) {
    ...
    ! function () {
      ...
      for ( i; i < l; i = i + 1 ) {
        ...
        if ( ! proof( lookup[ i ], extend( {}, condition ) ) ) continue;
