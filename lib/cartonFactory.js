...
! function () {
  ...
  function mix ( a, b, settings ) {
    ...
    var overwrite = settings.overwrite || true;
    var add = settings.add || true;
    var deep = settings.deep || true;
    var exclude = settings.exclude || '';
    var PX = settings.px || false;
    var HYP = settings.hyphenify || false;
  ...
  function px ( value ) { if ( typeof value === 'number' ) { return value + 'px'; }; return value; }
    ...
    while ( i-- ) {
      ...
      ! function ( pre ) {
          ...
          if ( matched !== null ) {
            ...
            while ( i-- ) {
              x += has( prefix.selector, name ) ? browser[ i ] : ''
              x += name + ':'
  ...
  function retinafy ( str ) {
    if ( ! isString( str ) || ! cartonSettings.retina ) return str;
  ...
  function init ( settings ) {
    ...
    if ( settings.extend ) mix( cartonSettings, settings.extend, { px: true, hyphenify: true } );
  ...
  Factory.prototype = {
    ...
    newCarton: function ( args ) {
      var obj = mix( { dom: this.dom, id: this.id }, args, { px: true, hyphenify: true } );
  ...
  function Carton ( obj, factory ) {
    mix( this, obj, { px: true, hyphenify: true } );
  ...
  Carton.prototype = {
    ...
    trigger: function ( type ) {
      if ( isFunction( callback[ type ] ) ) callback[ type ].call( null, this.parse(), type );
    }, 
    ...
    parse: function () {
      return {
        ...
        , align: this.parseSelector( 'align' + capitaliseFirstLetter( this.align ) )
        , styles: this.id + ( this.pseudo ? this.pseudo : ''  ) 
        , show: this.show === true  ? this.parseSelector( 'show' + capitaliseFirstLetter( this.type ) ) : ''
        }
        , type: this.type
        , pseudo: this.pseudo
    ...
    update: function ( setter ) {
      ...
      mix( this, setter, { px: true, hyphenify: true } );
      parsed = this.parse();
  ...
  function parse ( st ) {
    ...
    if (  isFunction( styleTag.appendChild ) ) {
  ...
  function find ( condition, lookup ) {
    ...
    ! function () {
      ...
      for ( i; i < l; i = i + 1 ) {
        ...
        if ( ! proof( lookup[ i ], mix( {}, condition, { px: true, hyphenify: true } ) ) ) continue;
    ...
    function proof ( carton, condition ) {
      ...
      for ( i in condition ) {
        ...
        conditionIsArray = isArray( condition[ i ] );
           existInCarton = ( ! isUndefined( carton[ i ] ) );
            isntABoolean = ( ! isBoolean( condition[ i ] ) );

        // if condition is an obj start recursion

        if ( ! conditionIsArray && isObject( condition[ i ] ) && ! condition[ i ].nodeType ) { 
        ...
        else if ( conditionIsArray && ! has( condition[ i ], carton[ i ] ) ) { lock = false; continue; }
