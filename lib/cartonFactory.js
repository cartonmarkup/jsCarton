...
! function ( help ) {
  ...
  var showGrid;
  var callback = {};
  var cartonKeys = [ 'type', 'align', 'show', 'styles' ];
  var cartonSettings = {
  ...
  Factory.prototype = {
    ...
    addSelector: function () {
      ...
      while ( i-- ) {
        ...
        ! function ( keys ) {
            ...
            for ( i; i < l; i = i + 1 ) {
              ...
              if ( ! parsed.styles[ keys[ i ] ] || keys[ i ] === 'styles' ) continue;

              name = parsed.classNames[ keys[ i ] ].replace( parsed.pseudo, '' );
      ...
      if ( parsed ) selector.push( ( use === '#' ) ?  parsed.id : parsed.classNames[ 'styles' ].replace( parsed.pseudo, '' ) );
      
      selector = ( use === '#' ) ? selector.join(' ') : selector.join(' ') + ' ';
      val = ( use === '#' ) ? selector : attr.replace( this.selector, selector );
