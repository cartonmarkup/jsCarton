  ...
  ! function  ( domo, help ) {
    ...
    function Carton ( document ) {
      ...
      this.EACH = function () {
        ...
        factory.suspendParser( true );
        
        if ( help.isObject( list ) ) {
      ...
      this.update = function ( node, settings, hard ) {
        var filterPseudos = hasPseudoClass( [ help.mix( {}, settings ) ] ); 
        var l = filterPseudos.length;
        var criteria;
        var i = 0;

        for ( i ;i < l; i = i + 1 ) {
          criteria = { dom: node, pseudo: false }

          if ( ! help.isEmpty( filterPseudos[ i ].query ) ) {

            criteria.query = filterPseudos[ i ].query;
          }

          if ( filterPseudos[ i ].pseudo ) { 

            criteria.pseudo = filterPseudos[ i ].pseudo;
          }

          carton.set( criteria, filterPseudos[ i ], hard ? { deep: false } : {} );
        }
      }
